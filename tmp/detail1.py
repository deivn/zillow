...
class DataOpt(object):
    ...
    def data_page(self, url, detail, origin):
        ...
        try:
            ...
            print("url--------"+url)
            self.detail_queue.srem("content", origin)
            house_type = self.get_info_by_keyword(source, 'Type:')
            _tmp = source.xpath('//div[@class="home-facts-at-a-glance-section"]//div[contains(text(), "Type")]/following-sibling::div/text()')
            detail['house_type'] = _tmp[0] if _tmp else (house_type if house_type else "")
            self.re_queue.sadd("data", json.dumps(detail, ensure_ascii=False))
            time.sleep(2)
...
#     for item in dataopt.re_queue.smembers("data"):
#         _json = json.loads(item)
#         if _json:
#             pid_url = re.findall(r'https://www.zillow.com/homedetails/\d+_zpid', _json['url'])
#             if pid_url:
#                 print(pid_url[0])
#                 i = {pid_url[0]: item.decode()}
#                 dataopt.re_queue.srem("data", item)
#                 dataopt.detail_queue.sadd("content", json.dumps(i, ensure_ascii=False))
#
#                 # dataopt.get_ip()
#                 # t.submit(dataopt.data_page, _json, item)
#
#         else:
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=8) as t:
        for item in dataopt.detail_queue.smembers("content"):
            _json = json.loads(item)
            if _json:
                for url, v in _json.items():
                    t.submit(dataopt.data_page, url, json.loads(v), item)
                    time.sleep(10)
...
def  _matchs():
    det_q = redis.Redis(host='47.106.140.94', port='6486')
    org_q = redis.Redis(host='47.106.140.94', port='6486', db=3, decode_responses=True)
    count = 0
    for item in det_q.smembers("data"):
        _json = json.loads(item)
        for orgin in org_q.smembers("content"):
            _match = json.loads(orgin)
            if _json['url'] == _match['detail_url'] and _json['house_type'] == 'FOR_RENT':
                count += 1
                det_q.srem("data", item)
                print("total: %d" % count)
                _json['house_type'] = _match['homeType']
                det_q.sadd("data", json.dumps(_json, ensure_ascii=False))
        else:
            continue

...
class Redis2Mysql(object):
    def __init__(self, city_path, housetype_path):
        ...
        self._redis01 = redis.Redis(host='47.106.140.94', port='6486', db=1, decode_responses=True)
...
def main():
    ...
    while True:
        ...
        # list = redis2mysql._redis01.lrange("data_copy", 0, -1)
        # for data in list:
        if data:
            redis2mysql._redis01.lpush("data_copy", data)
            ...
            if price != '0' and imgs and contact_phone and house_type_id and city_id and street.find("undisclosed") == -1:
                if lot_sqft or living_sqft:
                    ...
                    item['lot_sqft'] = lot_sqft
                    item['user_input_unit'] = str(item['lot_sqft']) + ' sqft'
                    item['living_sqft'] = living_sqft

...
class DataOpt(object):
    ...
    def get_yearbuild(self, source, keyword):
        yearbuild = source.xpath('//li[@class="ds-sub-section-container"]//td[contains(text(), "'+keyword+'")]/following-sibling::td/span/text()')
    ...
    def data_page(self, detail):
        browser = self.star_chr()
        if "url" in detail.keys():
            detail["zipcode"] = _json["detail"]["zipcode"]
            detail["latitude"] = _json["detail"]["latitude"]
            detail["longitude"] = _json["detail"]["longitude"]
            detail["city"] = _json["detail"]["city"]
            detail["state"] = _json["detail"]["state"]
        ...
        try:
            ...
            # Year built
            year_build = detail["yearBuilt"] if "yearBuilt" in detail.keys() else self.get_info_by_keyword(source, 'Year built:')
            if not year_build:
                year_build = self.get_yearbuild(source, 'Year built')
